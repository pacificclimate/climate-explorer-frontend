...
describe("parseBootstrapTableData", function () {
  it("correctly flattens a stats object for passing to the DataTable component", function () {
    var result = util.parseBootstrapTableData(
...
describe("parseWatershedTableData", function () {
  it("parses a watershed API response into attribute-value-units tuples", function () {
    var result = util.parseWatershedTableData(
      mockAPI.watershed,
      "POINT (-119.15625 53.09375)",
    );
...
describe("validateLongTermAverageData", function () {
  ...
  it("rejects data without units", function () {
    ...
    const func = () => {
      util.validateLongTermAverageData(noUnits);
    };
    expect(func).toThrow();
  });
...
describe("validateStatsData", function () {
  ...
  it("rejects NaN values", function () {
    ...
    const func = () => {
      util.validateStatsData({ data: nan });
    };
    expect(func).toThrow();
  });
...
describe("validateAnnualCycleData", function () {
  ...
  it("rejects concatenanted chronology data", function () {
    ...
    var concatenatedTasmaxTimeseries = JSON.parse(
      JSON.stringify(mockAPI.monthlyTasmaxTimeseries),
    );
    _.extend(
      concatenatedTasmaxTimeseries.data,
      mockAPI.seasonalTasmaxTimeseries.data,
    );
    _.extend(
      concatenatedTasmaxTimeseries.data,
      mockAPI.annualTasmaxTimeseries.data,
    );
    const func = () => {
      util.validateAnnualCycleData({ data: concatenatedTasmaxTimeseries });
    };
    expect(func).toThrow();
  });
...
describe("validateUnstructureTimeseriesData", function () {
  ...
  it("rejects an empty timeseries", function () {
    ...
    noTimestamps.times = {};
    const func = () => {
      util.validateUnstructuredTimeseriesData({ data: noTimestamps });
    };
    expect(func).toThrow();
  });
  it("accepts a valid timeseries", function () {
    ...
    expect(
      util.validateUnstructuredTimeseriesData({
        data: concatenatedTasmaxTimeseries,
      }),
    ).toEqual({ data: concatenatedTasmaxTimeseries });
  });
});
...
describe("validateWatershedData", function () {
  ...
  it("rejects watersheds with missing data", function () {
    ...
    _.forEach(attributes, function (att) {
      ...
      const func = function () {
        util.validateWatershedData({ data: missing });
      };
      expect(func).toThrow();
    });
  });
...
describe("timeKeyToResolutionIndex", function () {
  it("converts a time index into a resolution / index pairing", function () {
    expect(util.timeKeyToResolutionIndex(0)).toEqual({
      timescale: "monthly",
      timeidx: 0,
    });
    expect(util.timeKeyToResolutionIndex(16)).toEqual({
      timescale: "yearly",
      timeidx: 0,
    });
    expect(util.timeKeyToResolutionIndex(13)).toEqual({
      timescale: "seasonal",
      timeidx: 1,
    });
...
describe("timeResolutionIndexToTimeOfYear", function () {
  it("converts a time resolution + time index to a string", function () {
    ...
    expect(util.timeResolutionIndexToTimeOfYear("seasonal", 0)).toBe(
      "Winter-DJF",
    );
    expect(util.timeResolutionIndexToTimeOfYear("yearly", 0)).toBe("Annual");
    expect(util.timeResolutionIndexToTimeOfYear("daily", 200)).toBe(
      "daily 200",
    );
    expect(util.timeResolutionIndexToTimeOfYear("monthly", 45)).toBe(
      "monthly 45",
    );
  });
});
...
describe("extendedDateToBasicDate", function () {
  it("converts an extended format date to a basic format date", function () {
    expect(util.extendedDateToBasicDate("1997-01-15T00:00:00Z")).toBe(
      "1997-01-15",
    );
    expect(util.extendedDateToBasicDate("2030-12-18T23:16:59Z")).toBe(
      "2030-12-18",
    );
  });
});
...
describe("timestampToTimeOfYear", function () {
  it("converts timestamps into monthly values", function () {
    ...
    expect(
      util.timestampToTimeOfYear("1977-04-15T00:00:00Z", "monthly", false),
    ).toBe("April");
  });
  it("converts timestamps into seasonal values", function () {
    expect(
      util.timestampToTimeOfYear("1977-01-15T00:00:00Z", "seasonal", false),
    ).toBe("Winter-DJF");
    expect(
      util.timestampToTimeOfYear("1977-04-15T00:00:00Z", "seasonal", false),
    ).toBe("Spring-MAM");
    expect(
      util.timestampToTimeOfYear("1977-07-15T00:00:00Z", "seasonal", false),
    ).toBe("Summer-JJA");
    expect(
      util.timestampToTimeOfYear("1977-10-15T00:00:00Z", "seasonal", false),
    ).toBe("Fall-SON");
  });
  it("converts timestamps into annual values", function () {
    ...
    expect(
      util.timestampToTimeOfYear("1977-04-15T00:00:00Z", "yearly", true),
    ).toBe("Annual 1977");
  });
  it("does not convert unrecognized resolutions", function () {
    expect(
      util.timestampToTimeOfYear("1977-07-05T00:00:00Z", "daily", true),
    ).toBe("1977-07-05T00:00:00Z");
  });
});
...
describe("sameYear", function () {
  it("checks if dates happen during the same celandar year", function () {
    expect(
      util.sameYear("1977-01-01T00:00:00Z", "1977-12-31T00:00:00Z"),
    ).toBeTruthy();
    expect(
      util.sameYear("1977-12-31T00:00:00Z", "1978-01-01T00:00:00Z"),
    ).not.toBeTruthy();
  });
});
...
describe("capitalizeWords", function () {
  it("capitalizes the first letter of each word in a string", function () {
    expect(util.capitalizeWords("initial lowercase string")).toBe(
      "Initial Lowercase String",
    );
    expect(util.capitalizeWords("Initial uppercase")).toBe("Initial Uppercase");
    expect(util.capitalizeWords("string number the 3rd")).toBe(
      "String Number The 3rd",
    );
  });
});
...
describe("caseInsensitiveStringSearch", function () {
  ...
  it("does not find nonexistant substrings", function () {
    expect(
      util.caseInsensitiveStringSearch("category", "and"),
    ).not.toBeTruthy();
    expect(
      util.caseInsensitiveStringSearch("CATEGORY", "AND"),
    ).not.toBeTruthy();
...
describe("nestedAttributeIsDefined", function () {
  it("returns true when an attribute is defined", function () {
    expect(util.nestedAttributeIsDefined({ attribute: 0 }, "attribute")).toBe(
      true,
    );
    expect(
      util.nestedAttributeIsDefined(
        { attribute: { nested: 0 } },
        "attribute",
        "nested",
      ),
  ...
  it("returns false when an attribute is undefined", function () {
    ...
    expect(util.nestedAttributeIsDefined({ attribute: 0 }, "missing")).toBe(
      false,
    );
    expect(
      util.nestedAttributeIsDefined(
...
describe("WKTPointToGeoJSONPoint", function () {
  it("throws an error on bad strings", function () {
    ...
    _.forEach(bad_wkts, function (bad_wkt) {
      const func = function () {
        util.WKTPointToGeoJSONPoint(bad_wkt);
      };
      expect(func).toThrow();
    });
  });
  it("parses WKT Points", function () {
    const expected_geoJSON = {
      type: "Point",
      coordinates: [-119.15625, 53.09375],
    };
    expect(util.WKTPointToGeoJSONPoint("POINT (-119.15625 53.09375)")).toEqual(
      expected_geoJSON,
    );
  });
});
