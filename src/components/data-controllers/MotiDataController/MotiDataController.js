...
export default createReactClass({
  ...
  getData: function (props) {
    if (this.multiYearMeanSelected(props)) {
      ...
      var myTimeseriesPromise = this.getTimeseriesPromise(
        props,
        monthlyMetadata.unique_id,
      );
      ...
      .then((response) => {
        this.setState({
          statsData: parseBootstrapTableData(
            this.injectRunIntoStats(response.data),
            props.meta,
          ),
        });
      })
  ...
  render: function () {
    ...
    return (
      <div>
        <h3>
          {this.props.model_id} {this.props.variable_id} {this.props.experiment}
        </h3>

        <Panel>
          <Panel.Heading>
            <Panel.Title>{graphsPanelLabel}</Panel.Title>
          </Panel.Heading>
          <Panel.Body>
        ...
        <Panel>
          <Panel.Heading>
            <Panel.Title>{statsTableLabel}</Panel.Title>
          </Panel.Heading>
          <Panel.Body className={styles.data_panel}>
            <DataTable
              data={statsData}
              options={this.state.statsTableOptions}
            />
            <div style={{ marginTop: "10px" }}>
              <Button
                style={{ marginRight: "10px" }}
                onClick={this.exportDataTable.bind(this, "xlsx")}
              >
                Export To XLSX
              </Button>
