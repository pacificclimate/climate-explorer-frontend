...
const LeafletControlContainer = ({ children }) => (
  <span className="leaflet-touch">
    <span className="leaflet-control-container">{children}</span>
  </span>
);
    ...
    <LeafletControlContainer>
      <div className="leaflet-draw leaflet-control">
        <div className="leaflet-draw-section">
          <div className="leaflet-draw-toolbar leaflet-bar leaflet-draw-toolbar-top">
            <button
              className="leaflet-draw-draw-circlemarker"
...
const mapColourScaleControls = <span>(Colour Scale Bar)</span>;
...
export const mapPanelLabel = (
  <LabelWithInfo label="Data Map">
    <p>
      Map displaying data selected by Model, Emissions scenario, and
      Variable(s).
    </p>
    <p>
      Summary of map tools and other controls. (For details, see{" "}
      <Link to="/help/general">Help</Link>.)
    </p>
    <ul className={css.controlsList}>
      <li>{mapZoomControls}: Zoom map in and out.</li>
      <li>
        {mapPolygonDrawControls}: Draw polygons on the map. Polygons determine
        the extents over which spatial data averaging is performed.
      </li>
      <li>
        {mapPolygonEditControls}: Edit and delete polygons on the map. Polygons
        determine the extents over which spatial data averaging is performed.
      </li>
      <li>
      ...
      <li>{mapColourScaleControls}: Displays data value ⇄ colour mapping.</li>
      <li>
  ...
  <LabelWithInfo label="Data Map">
    ...
    <ul className={css.controlsList}>
      ...
      <li>
        ...
        location is added to the map a polygon will be rendered that shows the
        upstream area contributing streamflow to the selected location.
      </li>
      <li>{mapDeleteControl}: Remove outlet location from map.</li>
      <li>
        {mapSettingsControl}: Select which dataset(s) are displayed and how.
      </li>
      <li>{mapColourScaleControls}: Displays data value ⇄ colour mapping.</li>
...
export const TimeSelectorLabel = ({ temporalLabelPart }) => (
  <LabelWithInfo label={temporalLabelPart}>
    <p>
      Select the portion of the year over which data values are averaged before
      those portion averages are averaged over a multi-decadal period.
    </p>
  </LabelWithInfo>
);
...
export const PaletteSelectorLabel = ({ name }) => (
  <LabelWithInfo label={`${name} Colour Palette`}>
    <p>
      Select the set of colours to which data values are mapped for
...
export const colourMapTypeSelectorLabel = (
  <LabelWithInfo label="Colour Map Type">
    <p>
      Select how data values are mapped to colours. For details on linear and
      logarithmic mappings, see Help.
    </p>
  </LabelWithInfo>
);
...
export const datasetFilterPanelLabel = (
  <LabelWithInfo label="Dataset Filter">
    ...
    <p>
      ...
      graphs, and in a statistical summary table. Within each such presentation
      of data, you can independently select which of the matching datasets to
      display.
    </p>
  </LabelWithInfo>
);

export const gcmDefn = (
  <span>
    A{" "}
    <a
      href="http://www.ipcc-data.org/guidelines/pages/gcm_guide.html"
      target="_blank"
      rel="noopener noreferrer"
    >
      GCM (General Circulation Model)
    </a>{" "}
    is a numerical model representing physical processes in the atmosphere,
    ocean, cryosphere and land surface of the Earth. GCMs are the most advanced
    tools currently available for simulating the response of the global climate
    system to increasing greenhouse gas concentrations.
  </span>
);

export const modelSelectorLabel = (
  <LabelWithInfo label="Model">
    <p>GCM model with which the climate data was generated.</p>
    <p></p>
    {gcmDefn}
    <p>
      Models are identified by short codes. For full model identification, see{" "}
      <Link to="/help/general">Help</Link>.
    </p>
  </LabelWithInfo>
);
...
export const emissionScenarioSelectorLabel = (
  <LabelWithInfo label="Emissions Scenario">
    ...
    <p>
      ...
      greenhouse gas emissions, which are input into climate models. Higher
      RCP/SSP values represent greater projected greenhouse gas emissions. RCP
      and SSP scenarios are used for CMIP5 and CMIP6 experiments respectively.
    </p>
    <p>
      Emissions scenarios use the following coding. Note that all scenarios
      include <code>historical</code>, coupled with one of the future RCP/SSP
      emissions scenarios. For example <code>historical, rcp85</code> indicates
      an emissions scenario that uses historical data up to roughly the date of
      the model run, followed by the RCP 8.5 future emissions scenario from then
      until the final date of the model run.
    </p>
    <Table condensed>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            5-8.5 ("business as usual"; high emissions). This is the most
            appropriate sceanario to select for future planning.
          </td>
        </tr>
      </tbody>
    </Table>
...
export const variableSelectorLabel = (
  <LabelWithInfo label="Variable">
    <p>
      Variable to view on the map and in the graphs. For variable details, see{" "}
      <Link to="/help/general">Help</Link>.
    </p>
...
export const filteredDatasetSummaryPanelLabel = (
  <LabelWithInfo label="Filtered Datasets Summary">
    ...
    <p>
      Each row of the table represents a group of up to 3 datasets. We group
      datasets by Model Run, Start Date and End Date, and each such group is
      labelled accordingly as shown in the "Label in selectors" column. Datasets
...
export const datasetSelectorLabel = (
  <LabelWithInfo label="Dataset">
    ...
    <p>
      The map and the graph(s) have independently selected datasets. That is,
      they may be displaying different datasets simultaneously.
    </p>
    <p>
      On the map, use the {mapSettingsControl} button to access the Dataset
      selector. The selected dataset is shown in the legend in the lower left of
      the map.
    </p>
    <p>
...
const downloadFormats = (
  <span>
    Data may be downloaded in Microsoft Excel compatible format (XSLX) or in
    Comma Separated Values (CSV) format. For details on these formats, see{" "}
    <Link to="/help/general">Help</Link>.
  </span>
);
...
export const downloadGraphDataLabel = (
  <LabelWithInfo label={exportDataLabel}>
    ...
    <p>
      ...
      and content of the exported data, see <Link to="/help/general">Help</Link>
      .
    </p>
    <p>{downloadFormats}</p>
  </LabelWithInfo>
...
export const singleLtaTabLabel = (
  <LabelWithInfo label="Long Term Average">
    ...
    <p>{watershedDefn}</p>
    <p>
      The Time of Year selector only allows Annual values (i.e. all values
      represent the annual maximum design flow event).
    </p>
  </LabelWithInfo>
);
...
export const modelContextTabLabel = (
  <LabelWithInfo label="Model Context">
    <p>
      ...
      scenario and variable. The selected model run is highlighted. This puts
      the selected model into context with other similar model runs.
    </p>
    <p>{spatialAveragingDefn}</p>
  </LabelWithInfo>
);
...
export const changeFromBaselineTabLabel = (
  <LabelWithInfo label="Change from Baseline">
    <p>
      ...
      for averages over variable values the future periods 2010-2039, 2040-2069,
      and 2070-2099. Baseline is the average over the period 1981-2010.
    </p>
    <p>
      Absolute values, including baseline, are shown in upper graphs. Scale for
      absolute values is on left-hand vertical axis.
    </p>
    <p>
...
export const variableResponseTabLabel = (
  <LabelWithInfo label="Variable Response">
    <p>
      This graph shows the influnce of the secondary variable on the primary
      variable irrespective of time.
    </p>
    <p>
      It is composed from timeseries data with matching availability for both
      variables. Each point in time {t} with data from each variable ({t}, {V1}(
      {t})) and ({t}, {V2}({t})) appears as the scatterplot point ({V2}({t}),{" "}
      {V1}({t})).
    </p>
    <p>
      The secondary variable appears along the x axis as the explanatory
      variable; the primary variable appears along the y axis as the response
      variable.
    </p>
    <p>{spatialAveragingDefn}</p>
  </LabelWithInfo>
...
export const statsTableLabel = (
  <LabelWithInfo label="Statistical Summary">
    <p>
      This table presents a statistical summary of all the datasets matching the
      selected Model, Emissions Scenario, and Variable.
    </p>
    <p>
      The matching datasets are identified by Averaging Period and Model Run.
    </p>
    <p>
      Statistics are computed for averages of portions of the year selected by
      Time Of Year, which portions are then averaged over the averaging period.
    </p>
    <p>{spatialAveragingDefn}</p>
    <p>The table can be sorted by column content.</p>
    <ul>
...
export const exportStatsTableDataLabel = (
  <LabelWithInfo label={exportDataLabel}>
    ...
    <p>
      The data downloaded is that shown in the table. For details on the layout
      and content of the exported data, see <Link to="/help/general">Help</Link>
      .
    </p>
    <p>{downloadFormats}</p>
...
export const watershedTableLabel = (
  <LabelWithInfo label="Watershed Upstream Of Selected Point">
    ...
    <p>
      ...
      is the drainage area of the watershed, upstream of the selected point. The
      Melton Ratio is defined as the relief (maximum minus minimum elevation),
