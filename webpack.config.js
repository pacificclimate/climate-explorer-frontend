...
const GLOBALS = {
  ...
  TILECACHE_URL : JSON.stringify(process.env.TILECACHE_URL
      || 'http://tiles.pacificclimate.org/tilecache/tilecache.py'),
  ...
  CE_BASE_PATH: JSON.stringify(process.env.CE_BASE_PATH || ''),
};

const AUTOPREFIXER_BROWSERS = [ 'Chrome >= 35', 'Firefox >= 31',
    'Explorer >= 9', 'Opera >= 12', 'Safari >= 7.1', ];
...
module.exports = {
  ...
  // 'node' and 'externals' added as a fix to make xlsx.js work with Webpack.
  // See: https://github.com/SheetJS/js-xlsx/issues/285
  node : {
    fs : 'empty',
  },
  externals : [ {
